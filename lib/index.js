...
internals.load = recipe => {
  if (typeof recipe === 'string' || recipe.isHouraRecipe !== true) {
    try {
      ...
      Maite.cook(recipe, [internals.Houra, recipe.path]);
    } catch (err) {
  ...
  const res = Validate.recipeConfig(internals.Houra.recipeConfig.get('recipe'));
  ...
  return Glue.compose(manifest, {
    ...
    return server.initialize()
      .then(() => {
        ...
        server.log('info', `Houra succesfully initialized with the recipe : ${internals.Houra.recipe.name}@${internals.Houra.recipe.version}`);
      })
      .then(() => server);
  });
});
