...
internals.Houra.start = Promise.method((recipe, appPath) => {
  ...
  return Glue.compose(manifest, {
    ...
    return server.initialize()
      .then(() => {
        if (internals.Houra.recipeConfig.get('recipe:manifest:plugins')) {
          internals.Houra.recipeConfig.get('recipe:manifest:plugins').forEach((plugin) => {
